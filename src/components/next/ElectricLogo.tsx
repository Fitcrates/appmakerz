...
export default function ElectricLogo({ src, alt, className = '' }: ElectricLogoProps) {
  ...
  return (
    <div className={`relative aspect-square ${className}`}>
      {/* SVG FILTER: 4× feTurbulence (numOctaves=4) for rich distortion.
          Blended with color-dodge, then displacement. Lighter than
          numOctaves=10 but keeps the complex electric feel. */}
      <svg width="0" height="0" className="absolute" aria-hidden="true">
        <defs>
          <filter id="electric-logo-filter" colorInterpolationFilters="sRGB" x="-50%" y="-50%" width="200%" height="200%">
      ...
      <div className="relative h-full w-full min-w-[200px] min-h-[200px]">
        ...
        <div
          className="absolute inset-0 z-10 rounded-full border-2 border-[#5eead4]"
          style={{ filter: 'url(#electric-logo-filter)' }}
        />
